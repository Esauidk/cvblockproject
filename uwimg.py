...
lib = CDLL(os.path.join(os.path.dirname(__file__), "libuwimg_mac.so"), RTLD_GLOBAL)
...
set_image_above_sat = lib.set_image_above_sat
set_image_above_sat.argtypes = [IMAGE, c_int, c_float, c_float]
set_image_above_sat.restype = None

set_image_below_sat = lib.set_image_below_sat
set_image_below_sat.argtypes = [IMAGE, c_int, c_float, c_float]
set_image_below_sat.restype = None

isolate_white_pixels = lib.isolate_white_pixels
isolate_white_pixels.argtypes = [IMAGE, c_float, c_float]
isolate_white_pixels.restype = None

clamp_hues = lib.clamp_hues
clamp_hues.argtypes = [IMAGE, c_int]
clamp_hues.restype = None

center_board = lib.center_board
center_board.argtypes = [IMAGE, c_float]
center_board.restype = IMAGE

scan_for_corners = lib.scan_for_corners
scan_for_corners.argtypes = [IMAGE, c_float, POINTER(POINT)]
scan_for_corners.restype = c_float

rotate_45 = lib.rotate_45
rotate_45.argtypes = [IMAGE]
rotate_45.restype = IMAGE

minimize_board = lib.minimize_board
minimize_board.argtypes = [IMAGE, c_float]
minimize_board.restype = IMAGE

show_available_corners = lib.show_available_corners
show_available_corners.argtypes = [IMAGE, c_float]
show_available_corners.restype = None

show_illegal_squares = lib.show_illegal_squares
show_illegal_squares.argtypes = [IMAGE, c_float]
show_illegal_squares.restype = None

find_piece_placement_lib = lib.find_piece_placement
find_piece_placement_lib.argtypes = [IMAGE, c_float, c_char_p]
find_piece_placement_lib.restype = None

def find_piece_placement(i, f, c):
    return find_piece_placement_lib(i, f, c.encode('ascii'))
...
make_sat_filter = lib.make_sat_filter
make_sat_filter.argtypes = [c_int]
make_sat_filter.restype = IMAGE
